...
Target: 26 slides (ZH primary, EN parallel)
...
- 底部引用: [1] Anthropic Harness 文章 [2] OpenAI Harness Engineering

### Slide 3: System Architecture
- Two Pillars: Models (推理与理解) + Tools (28+ built-in + MCP + Skills + Hooks)
- Three Environments: Local / Cloud / Remote Control
- 底部: Claude Code = Agentic Harness built around Claude LLM

### Slide 4: Context is All You Need
...
## Part 2: 实践篇 (17 slides)

### Slide 5: Motivation — 现状
- CLI Agent 生态快速迭代，每周都在变
- 左: Community Voices — @bcherny, @trq212 [link], @systematicls, @HiTw93 [link], 胡渊鸣, AReaL
- 右: THE PROBLEM (不知道从哪里开始) + OUR APPROACH (围绕 Context 理解工具角色)

### Slide 6: Features Serve Context (callback)
...
### Slide 7: Skills — 可复用工作流的分发单元
- 左: What is a Skill? + Anthropic 内部几百个活跃 Skills
- 右: Popular Skills — notion-lifeos, frontend-design, proactive-agent, hf-paper-pages, superpowers
- 本质: 工作流的可重复化分发

### Slide 8: Skills 对研究者的价值
- 01: Context 满了 → 整理成 Skill
- 02: 重复 Workflow → 一键 Skill，持续迭代
- 03: 告别漫长 Prompt，维护 Skill 即可

### Slide 9: Skills 最佳实践
- 左: 三条写作原则 — description=触发条件 / gotchas=最有价值 / 渐进式披露
- 右: Anthropic 九大分类 (压缩版)
- Source: @trq212

### Slide 10: 新的重复劳动 (Hook 引入)
...
### Slide 11: Hooks — 确定性消灭重复劳动
- 官方文档引用: "user-defined shell commands, HTTP endpoints, or LLM prompts..."
- 左: 4 handler types (command/http/prompt/agent) + 右: 8+ key events
- 底部: matcher 正则 / exit code 机制 / 三层配置
- 金句: "把 Best Practice 从记忆变成机制"

### Slide 12: Hook 驱动的 Context 自动化
- 左: Input/Output/Manage 三维度对应 hook 场景
- 右: Case — 实验管理 (exp{NN}{x}/ 目录结构 + SessionStart/PostToolUse/Stop 自动化)
- 底部: 实验 pipeline (new-exp → scaffold → run → analyze → summary → commit)

### Slide 13: Sub-agent — Context Isolation 的功能分区
...
### Slide 17: Think — Brainstorming → Spec → Plan
...
- 引用: PostTrainBench 为 first user

---

## Part 3: 实用技巧 (3 slides)

### Slide 21: CLAUDE.md 最佳实践
...
### Slide 30: 参考阅读
- 左: Harness Engineering (Anthropic x2, OpenAI, Manus, Ivan Zhao) + Official Docs
- 右: Key People (@trq212, @bcherny, @systematicls, @HiTw93, AReaL) + GitHub (freemty/labmate, obra/superpowers, etc.)
